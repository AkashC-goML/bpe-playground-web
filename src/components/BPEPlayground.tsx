...
import { 
  Play, RotateCcw, BookOpen, Settings2, Brain, Eye, Zap, GitBranch, 
  Linkedin, FileText, Download, Upload, BarChart3, LineChart, 
  Layers, Cpu, Database, Share2, Info, AlertCircle, CheckCircle2,
  Hash, Target, Workflow, Code2, Globe, TrendingUp
} from 'lucide-react';
...
const TOKENIZER_MODELS: TokenizerModel[] = [
  ...
  {
    name: "GPT-4",
    description: "GPT-4 cl100k_base tokenizer with enhanced multilingual support",
    vocabSize: 100256,
    specialTokens: ["<|endoftext|>", "<|fim_prefix|>", "<|fim_middle|>", "<|fim_suffix|>"],
    icon: Zap,
...
const BPEPlayground = () => {
  ...
  const currentModel = TOKENIZER_MODELS.find(m => m.name === selectedModel) || TOKENIZER_MODELS[5];
  ...
  const computeBPE = (text: string, maxVocabSize: number, model: TokenizerModel): BPEResult => {
    ...
    if (model.name.includes("GPT")) {
      ...
      processedText = text.toLowerCase().replace(/[^\w\s]/g, ' ').replace(/\s+/g, ' ').trim();
    ...
    const uniqueChars = [...new Set(tokens.filter(t => !t.includes('</w>') && !t.includes('##')))];
    ...
    for (let iter = 1; iter <= maxIterations && currentVocabulary.length < maxVocabSize; iter++) {
      ...
      for (const [pair, freq] of pairCounts.entries()) {
        ...
        if (freq > maxFreq || (freq === maxFreq && pair.length < (mostFrequentPair ? mostFrequentPair[0] + mostFrequentPair[1] : '').length)) {
          maxFreq = freq;
    ...
    const averageTokenLength = finalTokens.reduce((sum, token) => sum + token.length, 0) / finalTokens.length;
        ...
        <div className="container mx-auto px-4 py-20 relative">
          <div className="text-center">
            <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm rounded-full px-6 py-3 mb-8">
              ...
              <span className="text-sm font-medium">Advanced Interactive BPE Explorer</span>
            </div>
            ...
            <p className="text-xl text-white/90 max-w-4xl mx-auto mb-8 leading-relaxed">
              Deep dive into tokenization with real-world model implementations. Compare GPT-4, BERT, T5, LLaMA 
              tokenizers with advanced statistics, compression analysis, and interactive visualizations.
            </p>
                  ...
                  >
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`p-2 rounded-lg ${
                        selectedModel === model.name ? 'bg-blue-500 text-white' : 'bg-blue-100 text-blue-600'
                      }`}>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-medium text-blue-600">{model.vocabSize.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-gray-500">Special Tokens:</span>
                        <span className="font-medium">{model.specialTokens.length}</span>
                      </div>
                    </div>
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {TOKENIZER_MODELS.filter(m => m.name !== "Custom").map((model) => (
                      <tr key={model.name} className="border-b hover:bg-white/50">
                        <td className="p-3 font-medium">{model.name}</td>
                        <td className="p-3 text-gray-600">{model.preprocessing}</td>
                        <td className="p-3 text-gray-600">{model.mergeStrategy}</td>
                        <td className="p-3">{model.vocabSize.toLocaleString()}</td>
                        <td className="p-3">
  ...
  <SelectContent>
    {SAMPLE_TEXTS.map((sample) => (
            ...
            <CardContent className="space-y-6">
              ...
              {currentBPEStep.mostFrequentPair && (
                <div className="bg-gradient-to-r from-blue-50 to-purple-50 p-4 rounded-lg border">
                  ...
                  <div className="text-xs text-gray-600 space-y-1">
                    ...
                    <div>Compression Ratio: {currentBPEStep.compressionRatio.toFixed(2)}x</div>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4 text-center">
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-lg border border-green-200">
                  ...
                  <div className="text-xs text-green-700 font-medium">Tokens</div>
                </div>
                <div className="bg-gradient-to-r from-blue-50 to-cyan-50 p-4 rounded-lg border border-blue-200">
                  <div className="text-2xl font-bold text-blue-600">
                    {currentBPEStep.vocabulary.length}
                  </div>
                  <div className="text-xs text-blue-700 font-medium">Vocab Size</div>
                </div>
              </div>

              {/* Compression History Mini Chart */}
              <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-lg border border-purple-200">
                <h5 className="text-sm font-medium mb-2 text-purple-700">Compression Progress</h5>
                <div className="flex items-end gap-1 h-12">
                  {compressionHistory.slice(0, 20).map((ratio, i) => (
                    <div
                      ...
                      style={{ height: `${(ratio / Math.max(...compressionHistory)) * 100}%` }}
                      title={`Step ${i}: ${ratio.toFixed(2)}x`}
                    />
                  ))}
                </div>
                <div className="text-xs text-purple-600 mt-1">
                  Current: {currentBPEStep.compressionRatio.toFixed(2)}x compression
                </div>
              </div>
            </CardContent>
          </Card>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-white/70 backdrop-blur-sm p-4 rounded-xl border border-emerald-200">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-medium text-emerald-700">Total Merges</span>
                  </div>
                  <div className="text-2xl font-bold text-emerald-600">
                    {bpeResult.statistics.totalMerges}
                  </div>
                  <div className="text-xs text-emerald-600 mt-1">
                    {((bpeResult.statistics.totalMerges / (bpeResult.steps.length || 1)) * 100).toFixed(1)}% efficiency
                  </div>
                </div>
                ...
                <div className="bg-white/70 backdrop-blur-sm p-4 rounded-xl border border-blue-200">
                  ...
                  <div className="text-xs text-blue-600 mt-1">
                    {((1 - 1/bpeResult.statistics.finalCompressionRatio) * 100).toFixed(1)}% size reduction
                  </div>
                </div>
                ...
                <div className="bg-white/70 backdrop-blur-sm p-4 rounded-xl border border-purple-200">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-medium text-purple-700">Avg Token Length</span>
                  </div>
                ...
                <div className="bg-white/70 backdrop-blur-sm p-4 rounded-xl border border-orange-200">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-medium text-orange-700">Vocab Efficiency</span>
                  </div>
                  <div className="text-2xl font-bold text-orange-600">
                    {(bpeResult.statistics.vocabularyEfficiency * 100).toFixed(1)}%
                  </div>
              ...
              <div className="mt-6 bg-white/70 backdrop-blur-sm rounded-xl p-6 border border-gray-200">
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {bpeResult.steps.slice(0, 10).map((step, i) => (
                        <tr key={i} className={`border-b border-gray-100 ${i === currentStep ? 'bg-blue-100' : ''}`}>
                          <td className="p-2 font-medium">{step.iteration}</td>
                          <td className="p-2">
                            {step.mostFrequentPair ? (
                              <span className="text-xs">
                                {step.mostFrequentPair[0]} + {step.mostFrequentPair[1]}
                              </span>
                            ) : '-'}
                          </td>
                          ...
                          <td className="p-2">{step.compressionRatio.toFixed(2)}x</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                {bpeResult.steps.length > 10 && (
                  <div className="text-xs text-gray-500 mt-2 text-center">
                    Showing first 10 steps of {bpeResult.steps.length} total steps
                  </div>
                )}
              </div>
            </CardContent>
                ...
                <div className="bg-gradient-to-r from-gray-50 to-blue-50 p-6 rounded-xl border border-gray-200 min-h-[150px]">
                  <div className="flex flex-wrap gap-2">
                    {currentBPEStep.mergedTokens.map((token, index) => {
                      ...
                      const isNewlyMerged = currentBPEStep.mostFrequentPair && 
                        token === currentBPEStep.mostFrequentPair[0] + currentBPEStep.mostFrequentPair[1];
                      ...
                      return (
                        <Badge 
                          ...
                          className={`text-xs transition-all duration-300 ${
                            isSpecialToken ? "bg-red-100 text-red-700 border-red-200" :
                            isEndToken ? "bg-blue-100 text-blue-700 border-blue-200" :
                            isNewlyMerged ? "bg-green-100 text-green-700 border-green-300 shadow-md animate-pulse" :
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-lg border border-green-200">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-medium text-green-700">Efficiency Metrics</span>
                    </div>
                    <div className="space-y-1 text-xs text-green-600">
                      <div>Total tokens: {currentBPEStep.mergedTokens.length}</div>
                      <div>Compression: {currentBPEStep.compressionRatio.toFixed(2)}x</div>
                      <div>Size reduction: {((1 - 1/currentBPEStep.compressionRatio) * 100).toFixed(1)}%</div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-blue-50 to-cyan-50 p-4 rounded-lg border border-blue-200">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-medium text-blue-700">Token Types</span>
                    </div>
                    <div className="space-y-1 text-xs text-blue-600">
                      <div>Special: {currentBPEStep.mergedTokens.filter(t => currentModel.specialTokens.includes(t)).length}</div>
                      <div>Subwords: {currentBPEStep.mergedTokens.filter(t => t.includes('##') || t.includes('</w>')).length}</div>
                      <div>Regular: {currentBPEStep.mergedTokens.filter(t => !currentModel.specialTokens.includes(t) && !t.includes('##') && !t.includes('</w>')).length}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <h4 className="text-sm font-medium mb-3 flex items-center gap-2">
                    ...
                    Base Characters & Special Tokens ({currentBPEStep.vocabulary.filter(token => 
                  ...
                  <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-lg border border-green-200">
                    <div className="flex flex-wrap gap-1 mb-2">
                      {currentBPEStep.vocabulary
                        ...
                        .slice(0, 20)
                        .map((token, index) => {
                          const isNewlyLearned = currentBPEStep.mostFrequentPair && 
                            token === currentBPEStep.mostFrequentPair[0] + currentBPEStep.mostFrequentPair[1];
                          return (
                            <Badge 
                              key={`merged-${index}`} 
                  ...
                  <div>
                    ...
                    <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-lg border border-purple-200 max-h-32 overflow-y-auto">
                      <div className="space-y-1">
                        {currentBPEStep.mergingRules.slice(-5).reverse().map((rule, index) => (
      ...
      <div className="mt-12 bg-gradient-to-r from-slate-900 to-blue-900 text-white rounded-2xl p-8 shadow-2xl">
        <h3 className="text-2xl font-bold mb-8 text-center">About the Creator & Learning Resources</h3>
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-white/80 text-sm">ML Engineer & AI Researcher</p>
              </div>
            </div>
            <p className="text-white/90 text-sm mb-4 leading-relaxed">
              Passionate about making complex AI concepts accessible through interactive tools and educational content. 
              Specializes in NLP, transformer architectures, and tokenization algorithms.
            </p>
            <div className="flex gap-3">
              <a 
                href="https://www.linkedin.com/in/akaszhu/" 
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/70 text-xs mt-1">
                      Deep dive into BPE algorithm with practical examples and implementation details
                    </p>
            ...
            <div className="space-y-3">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/70 text-xs">HuggingFace Official Tutorial</p>
                  </div>
                </div>
              </a>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/70 text-xs">Advanced Implementation Guide</p>
                  </div>
                </div>
              </a>
              ...
              <div className="mt-4 p-3 bg-yellow-500/20 rounded-lg border border-yellow-500/30">
                ...
                <p className="text-yellow-100 text-xs mt-1">
                  Run these notebooks to experiment with real tokenizer training on your own data!
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="mt-8 text-center">
          <p className="text-white/80 text-sm">
            This tool is open for educational use. Contribute to the project or suggest improvements!
          </p>
